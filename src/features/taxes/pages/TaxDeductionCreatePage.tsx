...
import { useCreateTaxDeduction, useTaxCategories } from '../hooks/useTaxes'
import DeductionForm from '../components/DeductionForm'

export default function TaxDeductionCreatePage() {
  ...
  const handleSubmit = async (data: Parameters<typeof createMutation.mutateAsync>[0]) => {
    try {
      ...
      toast.success('Deduccion creada exitosamente')
      navigate(`/taxes/deductions/${result.id}/edit`)
    } catch (err: any) {
      toast.error(err?.message || 'Error al crear la deduccion')
    }
  }
    ...
    <div className="relative space-y-6 pb-8">
      ...
      <div className="relative">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">Nueva Deduccion</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">Registra un gasto fiscalmente deducible</p>
          </div>
        </div>
