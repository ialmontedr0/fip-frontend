...
import {
  LayoutDashboard, Tags, TrendingUp, AlertTriangle, Lightbulb,
  Activity, Shield, PiggyBank, Cpu, ChevronDown,
} from 'lucide-react'

const NAV_ITEMS = [
  { href: '/ai/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/ai/classify', label: 'Clasificar', icon: Tags },
  { href: '/ai/predict', label: 'Predecir', icon: TrendingUp },
  { href: '/ai/anomalies', label: 'Anomalias', icon: AlertTriangle },
  { href: '/ai/recommendations', label: 'Recomendaciones', icon: Lightbulb },
  { href: '/ai/habits', label: 'Habitos', icon: Activity },
  { href: '/ai/risks', label: 'Riesgos', icon: Shield },
  { href: '/ai/savings', label: 'Ahorros', icon: PiggyBank },
  { href: '/ai/models', label: 'Modelos', icon: Cpu },
]

function ActivePill({ style }: { style: React.CSSProperties }) {
  return (
    <span
      className="absolute inset-y-1.5 left-0 rounded-xl bg-gradient-to-r from-purple-500/20 to-indigo-500/20 border border-purple-200/50 dark:border-purple-500/30 shadow-lg shadow-purple-500/10 transition-all duration-400 ease-out z-0"
      style={style}
    />
  )
}
...
function DesktopNav() {
  ...
  const getMatch = (href: string) => location.pathname.startsWith(href)
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-gray-100/80 dark:border-gray-700/80 shadow-sm">
              ...
              return (
                ...
                >
                  <div className={cn(
                    ...
                    active ? 'bg-gradient-to-br from-purple-500/20 to-indigo-500/20' : 'bg-gray-100 dark:bg-gray-700/50',
                  )}>
                    <ItemIcon className={cn('h-3.5 w-3.5', active ? 'text-purple-600 dark:text-purple-400' : '')} />
                  </div>
                  <span>{item.label}</span>
                  {active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-gradient-to-br from-purple-500 to-indigo-500" />}
                </button>
              )
            })}
          </div>
