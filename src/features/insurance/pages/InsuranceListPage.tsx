...
import { Plus, Search, SlidersHorizontal, ShieldCheck } from 'lucide-react'
import { cn } from '@/lib/utils'
import { useInsuranceList, useInsuranceDashboard } from '../hooks/useInsurance'
import InsuranceDashboardCards from '../components/InsuranceDashboardCards'
import InsuranceCard from '../components/InsuranceCard'
import { INSURANCE_STATUSES, INSURANCE_TYPES } from '@/types/insurance'
...
export default function InsuranceListPage() {
  ...
  const { data: dashboard } = useInsuranceDashboard()
    ...
    <div className="relative space-y-6 pb-8">
      ...
      {dashboard && !isLoading && (
        <div className="relative animate-fade-in">
          <InsuranceDashboardCards dashboard={dashboard} />
        </div>
      )}
      ...
      <div className="relative animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">Seguros</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {totalCount > 0 ? `${totalCount} seguro${totalCount !== 1 ? 's' : ''}` : 'Gestiona tus seguros'}
              </p>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((insurance, idx) => (
                ...
                >
                  <InsuranceCard insurance={insurance} index={idx} />
                </div>
              ))}
            </div>
          </>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center bg-white dark:bg-gray-800/80 rounded-2xl border border-gray-100 dark:border-gray-700/50 shadow-sm">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">
              {searchQuery ? `Sin resultados para "${searchQuery}"` : 'No tienes seguros aun'}
            </h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm mb-6">
              {searchQuery ? 'Intenta con otros terminos de busqueda' : 'Agrega tu primer seguro para dar seguimiento a tus polizas y primas'}
            </p>
            {!searchQuery && (
              <button
                type="button"
