...
import type { CreateInsuranceRequest } from '@/types/insurance'

export default function InsuranceCreatePage() {
  const navigate = useNavigate()
  const createMutation = useCreateInsurance()

  const handleSubmit = async (data: CreateInsuranceRequest) => {
    try {
      const result = await createMutation.mutateAsync(data)
      toast.success('Seguro creado exitosamente')
      navigate(`/insurance/${result.id}`)
    } catch (err: any) {
      toast.error(err?.message || 'Error al crear el seguro')
    }
  }
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Registra un nuevo seguro en tu cartera
            </p>
          </div>
        </div>
