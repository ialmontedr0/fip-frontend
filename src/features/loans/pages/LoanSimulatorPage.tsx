...
import {
  ...
  TrendingUp, PiggyBank, Target, Sparkles, Plus,
  BarChart3,
} from 'lucide-react'
...
export default function LoanSimulatorPage() {
  ...
  const handleSimulate = async (e: React.FormEvent) => {
    ...
    if (!principal || principal <= 0) {
      toast.error('Ingrese un monto principal valido')
      return
    }
    if (!rate || rate <= 0) {
    ...
    try {
      const res = await simulateMutation.mutateAsync({
        ...
        extra_monthly_payment: extraMonthlyPayment ? parseFloat(extraMonthlyPayment) : undefined,
      })
      setResult(res)
    } catch (err: any) {
  ...
  const hasExtra = result && result.extra_monthly_payment > 0
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Calcula pagos, intereses y costos totales
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="relative animate-fade-in">
          <div className="bg-white dark:bg-gray-800/80 rounded-2xl border border-gray-100 dark:border-gray-700/50 shadow-sm p-6">
            ...
            <form onSubmit={handleSimulate} className="space-y-5">
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Monto Principal *</label>
                <div className="relative">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      placeholder="0.00"
                      className="w-full px-4 pr-8 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/80 text-gray-900 dark:text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition-all"
                    />
                  </div>
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Pago Extra Mensual</label>
                  <div className="relative">
                    <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-sm text-gray-400 font-medium">$</span>
                    <input
                      type="number"
          ...
          {result ? (
            <div className="space-y-4">
              <div className="bg-white dark:bg-gray-800/80 rounded-2xl border border-gray-100 dark:border-gray-700/50 shadow-sm p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  ...
                  <ResultCard icon={Target} label="Costo Total" value={formatCurrency(result.total_cost)} color="text-gray-900 dark:text-gray-100" />
                  <ResultCard icon={Percent} label="Interes/Principal" value={`${(result.interest_to_principal_ratio * 100).toFixed(1)}%`} color="text-purple-600 dark:text-purple-400" />
                  <ResultCard icon={Calendar} label="Plazo Original" value={`${result.actual_months} meses`} />
                  {hasExtra && <ResultCard icon={TrendingUp} label="Plazo con Extra" value={`${result.early_payoff_months} meses`} color="text-emerald-600 dark:text-emerald-400" />}
                  {hasExtra && <ResultCard icon={DollarSign} label="Interes Ahorrado" value={formatCurrency(result.interest_saved_with_extra)} color="text-emerald-600 dark:text-emerald-400" />}
                </div>
              </div>
              ...
              {result.schedule_preview && result.schedule_preview.length > 0 && (
                <div className="bg-white dark:bg-gray-800/80 rounded-2xl border border-gray-100 dark:border-gray-700/50 shadow-sm p-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Vista Previa</h3>
                    <span className="text-xs text-gray-400 ml-auto">Primeros {result.schedule_preview.length} meses</span>
                  </div>
                  <div className="h-48">
                    <AmortizationChart
                      entries={result.schedule_preview.map((e) => ({
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-gray-100 dark:border-gray-700/50">
                          ...
                          <th className="text-right py-2 px-2 text-gray-500 dark:text-gray-400 font-medium">Pago</th>
                          <th className="text-right py-2 px-2 text-gray-500 dark:text-gray-400 font-medium">Principal</th>
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 px-5 py-3 text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-600 rounded-xl hover:from-emerald-600 hover:to-teal-700 shadow-lg shadow-emerald-500/25 hover:shadow-xl transition-all duration-200"
              >
                <Plus className="h-4 w-4" />
                Crear Prestamo con estos datos
              </button>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center bg-white dark:bg-gray-800/80 rounded-2xl border border-gray-100 dark:border-gray-700/50 shadow-sm p-6">
              ...
              <p className="text-sm text-gray-400 dark:text-gray-500 mt-1 max-w-sm leading-relaxed">
                Ingresa el monto, tasa de interes y plazo para simular tu prestamo. Los resultados apareceran aqui con graficos y detalle de pagos.
              </p>
            </div>
          )}
        </div>
